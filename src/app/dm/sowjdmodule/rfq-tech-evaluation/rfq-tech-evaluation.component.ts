...
@Component({
  selector: 'app-rfq-tech-evaluation',
  templateUrl: './rfq-tech-evaluation.component.html',
  styleUrls: ['./rfq-tech-evaluation.component.scss'],
  providers: [DatePipe],
})
export class RfqTechEvaluationComponent {
  ...
  ngOnInit(): void {
    this.getsowjdRFQTechEvaluation();
  ...
  getsowjdRFQTechEvaluation() {
    ...
    this.sowjdservice.getsowjdRFQTechEvaluation().subscribe({
      next: (res: any) => {
  ...
  sowjdRFQTechEvaluation(params: GridReadyEvent) {
  ...
  // }
  getRFQColumns() {
    this.columnSettingsService.getRFQColumns().subscribe((item: any) => {
  ...
  saveRFQColumns() {
    if (this.selectedColumns) {
      ...
      this.columnSettingsService
        .saveRFQColumns(this.selectedColumns)
        .subscribe((item: any) => {
  ...
  unSelectAllColumns() {
    ...
    this.selectedColumns = ['rfqNumber'];
    this.adjustWidth();
  }
  //Auto-Width Fix -start
  onFirstDataRendered(event: any) {
    this.adjustWidth();
    this.planningService.getColFilters(61).subscribe({
      next: (res: any) => {
        let onlyKeysarr = [];
