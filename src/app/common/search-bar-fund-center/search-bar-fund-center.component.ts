...
@Component({
  selector: 'app-search-bar-fund-center',
  templateUrl: './search-bar-fund-center.component.html',
  styleUrls: ['./search-bar-fund-center.component.scss'],
  encapsulation: ViewEncapsulation.None,
})
export class SearchBarFundCenterComponent implements OnInit, OnDestroy {
  @Input()
  ...
  ngOnInit() {
    this.getIntialFundCenter();
  }

  getIntialFundCenter(searchValue?: string) {
    if (
      this.companyId &&
      ...
      this.createsowjdformservice
        .getFundCenter(this.companyId, searchValue)
        .subscribe(
          (groups: any) => {
  ...
  getSearchValue(e: any) {
    this.getIntialFundCenter(e.target.value);
  }
