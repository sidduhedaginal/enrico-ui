...
import { PlanningImportedData } from '../../services/planningimporteddata.service';
import { dateFieldsForImports, hiddenColumns } from 'src/app/master/constants/app-constant';

@Component({
  selector: 'app-importedfile',
  templateUrl: './importedfile.component.html',
  styleUrls: ['./importedfile.component.css'],
  providers:[DatePipe]
})
export class ImportedfileComponent implements OnInit {
  ...
  constructor(
    ...
    private importdataService: PlanningImportedData,
  ...
  ngOnInit(): void {
    ...
    if (routerState) {
      ...
      this.SecondLevelId = routerState.metadataid;
    }
  ...
  saveData(flag: number) {
    ...
    this.apiService
      .post(
        `api/planning/import-data?flag=${flag}&companyCode=${this.companyCode}&featureCode=SecondLevelPlanning`,
        selectedData
      )
      .subscribe(
        (response: any) => {
          ...
          for(let item of rowDataModified){
            item.validityEnd = this.datePipe.transform(item.validityEnd, 'dd-MMM-yyyy');
            item.validityStart = this.datePipe.transform(item.validityStart, 'dd-MMM-yyyy');
