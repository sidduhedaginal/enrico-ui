...
@Component({
  selector: 'app-importresource',
  templateUrl: './importresource.component.html',
  styleUrls: ['./importresource.component.css']
})
export class ImportresourceComponent  {
  ...
  constructor(
    ...
    public dialogRef: MatDialogRef<ImportresourceComponent>,
  ...
  Onsave() {
    ...
    this.planningservice
      .post(
        `api/planning/supply-planning-import-file?featureCode=SecondLevelPlanning`,
        {
          flag: insertAndUpdate,
          fileName: this.filename,
          fileBase64: this.importdataService.getBase64Format(),
          Id:this.data.aopSlId
        }
      )
      .subscribe(
        (response: any) => {
          ...
          if (response.status == 'Record added successfully') {
            this.onNoClick();
            this.router.navigate(['planning/second-level-details/importedfile'], {
              state: {
                routerdata: JSON.stringify('Supply_Planning'),
                responsedata: JSON.stringify(response.data),
                metadataid:(this.data.aopSlId)
              },
            });
          } else {
