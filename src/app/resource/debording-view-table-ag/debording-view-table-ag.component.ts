...
@Component({
  selector: 'app-debording-view-table-ag',
  templateUrl: './debording-view-table-ag.component.html',
  styleUrls: ['./debording-view-table-ag.component.css']
})
export class DebordingViewTableAgComponent implements OnInit {
 ...
 // showCompleteEclBtn: boolean = false;
  ...
  public defaultColDef: ColDef = {
   ...
   columnDefs: any = [
    { headerName: "De-boarding ID ", 
    field: "deboardingRequestID",
    suppressDragLeaveHidesColumns: false,
    suppressMovable:true,
    pinned: 'left',
    cellStyle: this.cellStyling,hide: false },
    { 
    ...
    { 
      ...
      hide: false },
    {
      headerName: "Replacement Requested ", field: "isReplaceRequest", valueGetter: function (params) {
        if (params.data.isReplaceRequest == false) { return 'No'; }
        else { return 'Yes'; }
      },
      hide: false
    },
    {
      headerName: "Last Working Date", field: "lastWorkingDay", valueFormatter: function (params) {
        return moment(params.value).format('DD-MMM-yyyy');
      },cellClass: 'lwdDateFormat', hide: false,
      filterParams: {
    ...
    { 
      ...
      hide: false },
    {
      headerName: "Status", field: "statusDescription", valueGetter: function (params) {
        if (params.data.statusDescription == 'Submitted') {
          return 'De-boarding Initiated';
        } else {
          return params.data.statusDescription;
        }
      },
      hide: false
    },
  ...
  ngOnInit() {
  // if(localStorage.getItem('FilterColumnSttingsPopupCheckedUncheckedDB')){
  // this.columnDefs=JSON.parse(localStorage.getItem('FilterColumnSttingsPopupCheckedUncheckedDB'));
  // }
    this.paginationPageSize = 5;
    this.initNext();
    //this.getUserRolesInfo();
   // this.getRolePermission();
    this.getDeboardRecord();
  }
 ...
 // }
  ...
  getdeafultcolmuns(){
    if(this.vendorId){
      this.vendorService.getdeafultcolmunsVendor(65,this.vendorId)
      .subscribe((item: any) => {
      ...
      this.planningService.getdeafultcolmuns(65)
      .subscribe((item: any) => {
        this.savedColumns = item.data;
        if (this.savedColumns) {
          let savedColumnsArray = this.savedColumns.split(',');
  ...
  getDeboardRecord() {
    ...
    this.API.getDeboarding().subscribe((response: any) => {
      ...
      if (allData && allData.length > 0) {
        if ((this.userDetailsRoles == "/Vendors")||(this.userDetailsRoles == '/EnricoUsers' && this.permissionsBehaviorSubjectDeboarding?.readPermission)) {
  ...
  actionRenderer(params: any) {
    ...
    if (element.statusDescription == 'Submitted' && this.userDetailsRoles == '/EnricoUsers') {
      let _htmlContent = '<div class="cfcycleicons">&nbsp;';  
        if (this.permissionsBehaviorSubjectDeboarding?.approvePermission && ((element.firstApprover == this.userDetails.employeeNumber && element.isFirstLevelApproved == false) || (element.secondApprover == this.userDetails.employeeNumber && element.isFirstLevelApproved == true)  )) {
        _htmlContent += '<span class="submit" title="Approve">&nbsp;</span>&nbsp;'
      }
      if (this.permissionsBehaviorSubjectDeboarding?.rejectPermission) {
        _htmlContent += '<span class="sendback_planning" title="Send back/Reject">&nbsp;</span>&nbsp;'
      }
      if (this.permissionsBehaviorSubjectDeboarding?.delegatePermission) {
  ...
  onCellClicked(params: any) {
    if (params.column.colId === 'deboardingRequestID') {
      let element: any = [];
      element = params.data;
      this.deBoardIDDetails(element);
    }
    else if (params.column.colId === 'action') {
      let element: any = [];
      element = params.data;
      if (params.event.target.className == 'submit') {
        this.deboardRemarksActiondialog(element, 'Approve');
      }
      else if (params.event.target.className == 'sendback_planning') {
        this.deboardRemarksActiondialog(element, 'Reject');
      }
      else if (params.event.target.className == 'planning_delegate') {
        this.deboardRemarksActiondialog(element, 'Delegate');
      }
      else if (params.event.target.className == 'withdraw') {
        this.deboardRemarksActiondialog(element, 'Withdraw');
      }
    }
  }


  deboardRemarksActiondialog(element: any, type: any) {
    let _obj = {
      rowData: element,
      type: type,
    };
  ...
  //   this.adjustWidth();
  //   let onlyKeysarr=[];
  //   onlyKeysarr=JSON.parse(localStorage.getItem('filterModelDB'));
  //   if(onlyKeysarr){
  //  this.finalKeyArray =Object.keys(onlyKeysarr) ;
  //   }
  //   //-----------
  //   localStorage.getItem('DBChipsKey')
  //   this.gridOptions.api.setFilterModel(onlyKeysarr);
  
  // }
//   onFilterChanged(event: any) {   
//     const filterModel = event.api.getFilterModel();
//     localStorage.setItem('filterModelDB', JSON.stringify(filterModel));
//     let onlyKeysarr=[];
//     onlyKeysarr=JSON.parse(localStorage.getItem('filterModelDB'));
//     this.finalKeyArray =Object.keys(onlyKeysarr) ;
//     this.adjustWidth();
// //------------
//     localStorage.setItem('DBChipsKey',this.finalKeyArray)
...
//   }
  ...
  //     localStorage.setItem('filterModelDB', JSON.stringify(onlyKeysarr));
  //     let onlyKeysarr2=[];
  //     onlyKeysarr2=JSON.parse(localStorage.getItem('filterModelDB'));     
    ...
    if (this.selectedColumns) {
      if(this.vendorId){
        ...
        this.planningService
          .saveColSettings(65,this.selectedColumns)
          .subscribe((item: any) => {
            if (item.status === 'success') {
              this.setActiveItem('');
              this.loaderService.setDisableLoading();
  ...
  exportAgBtn() {
    ...
    var params = {
      fileName: `Deboarding_Export_`+ moment(new Date()).format('DD-MMM-YYYY'),// `DeboardTable.xlsx`,
    };
  ...
  columnFilters : any;
    ...
    onFirstDataRendered(event: any) {
      this.adjustWidth();
      if(this.vendorId){
        this.vendorService.getColFiltersVendor(65,this.vendorId).subscribe({
          next: (res: any) => {
            let onlyKeysarr = [];
        ...
        this.planningService.getColFilters(65).subscribe({
          next: (res: any) => {
            let onlyKeysarr = [];
            if (res.data.columnFilters == '') {
              this.filterChipsSets = null;
