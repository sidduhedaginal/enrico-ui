        ...
        for (let i = 0; i < nodeArray.length; i++) {
          if (nodeArray[i].nodeName === 'BUTTON') {
            if (nodeArray[i].innerHTML.length > 100 && nodeArray[i].disabled) {
              ...
              this.ren.setStyle(nodeArray[i], 'background-color', '#007BC0');
...
let _numberOfPages=this.matPag.pageIndex==0?1:this.numOfPages;
    for (let i = 0; i < _numberOfPages; i++) {
   ...
   const  linkBtn: MatButton = this.ren.createElement('button');
    ...
    switch (i) {
      ...
      case this._pageGapTxt:
        ...
        if (pageIndex == this.lastPageIndex) {
        //  this.ren.setAttribute(linkBtn, 'disabled', 'disabled');//commented this linne
        }
        break;
      default:
        this.ren.listen(linkBtn, 'click', () => {
    ...
    this._buttons.push(linkBtn);//(linkBtn)
    return linkBtn;
  }
  ...
  private calcRangeEnd(middleIndex: number): number {
    switch (true) {
      ...
      case this._curPageObj.pageIndex < this._curPageObj.previousPageIndex &&
        this._curPageObj.pageIndex < middleIndex &&
        this._rangeStart >= 0 &&
        this._rangeEnd > this._showTotalPages-1 : //remove -1
        return this._rangeEnd - 1;
      default:
