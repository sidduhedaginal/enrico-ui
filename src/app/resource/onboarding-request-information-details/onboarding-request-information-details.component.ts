...
@Component({
  selector: 'app-onboarding-request-information-details',
  templateUrl: './onboarding-request-information-details.component.html',
  styleUrls: ['./onboarding-request-information-details.component.scss'],
})
export class OnboardingRequestInformationDetailsComponent implements OnInit {
  showLoading: boolean = false;
  sscDocuments: any = ['SSC Document1.pdf', 'SSC Document2.pdf', 'SSC Document3.pdf'];
  highestQualificationDocuments: any = ['HQ Document1.pdf', ' HQ Document2.pdf', 'HQ Document3.pdf'];
  remarksList: any = [];
  getElementData: any = [];
  _checkuser: any = [];
  ...
  forDownloadSSNtoken: string = "sv=2022-11-02&se=2024-05-16T10%3A10%3A25Z&sr=c&sp=r&sig=RtwAhr47nBKX1bFsD88SBMpc0EpO3L%2BtQ54PQrS8c00%3D";
  ...
  addressProofSectionRemarksModel:any="";
  residenceSectionRemarksModel:any="";
  sscSectionRemarksModel:any="";
  hqSectionRemarksModel:any="";
  workExpSectionRemarksModel:any="";
  previousWorkedBoschSectionRemarksModel:any="";
  photoSectionRemarksModel:any="";
  purchaseOrderSectionRemarksModel:any="";
louSectionRemarksModel:any="";
bvrSectionRemarksModel:any="";
ispTrainingSectionRemarksModel:any="";
parentOrgSectionRemarksModel:any="";
dooSectionRemarksModel:any="";
ispCertificateSectionRemarksModel:any="";
releivingSectionRemarksModel:any="";
loaSectionRemarksModel:any="";
  ...
  isShowAddreessproofInforemarksButtonSaveCancel:boolean=false;
  isShowResidenceRemarksButtonSaveCancel:boolean=false;
  isShowSSCRemarksButtonSaveCancel:boolean=false;
  isShowHQRemarksButtonSaveCancel:boolean=false;
  ...
  isShowPurchaseOrderRemarksButtonSaveCancel:boolean=false;
isShowLOURemarksButtonSaveCancel:boolean=false;
isShowBVRRemarksButtonSaveCancel:boolean=false;
isShowIspTrainingRemarksButtonSaveCancel:boolean=false;
isShowParentOrgRemarksButtonSaveCancel:boolean=false;
isShowDOORemarksButtonSaveCancel:boolean=false;
isShowIspCertificateRemarksButtonSaveCancel:boolean=false;
isShowRelievingRemarksButtonSaveCancel:boolean=false;
isShowLOARemarksButtonSaveCancel:boolean=false;
  ...
  showHideAddressProofDetailsRemarksForSendBack:boolean=false;
  showHideResidenceRemarksForSendBack:boolean=false;
  showHideSSCRemarksForSendBack:boolean=false;
  showHideHQRemarksForSendBack:boolean=false;
  ...
  showHidePurchaseOrderRemarksForSendBack:boolean=false;
  showHideLOURemarksForSendBack:boolean=false;
  showHideBVRRemarksForSendBack:boolean=false;
  showHideIspTrainingRemarksForSendBack:boolean=false;
  showHideParentOrgRemarksForSendBack:boolean=false;
  showHideDOORemarksForSendBack:boolean=false;
  showHideIspCertificateRemarksForSendBack:boolean=false;
  showHideRelievingRemarksForSendBack:boolean=false;
  showHideLOARemarksForSendBack:boolean=false;
  ...
  photoSection:any=[];
  purchaseOrderSection:any=[];
  louSection:any=[];;
  bvrSection:any=[];
  ispTrainingSection:any=[];
  parentOrgSection:any=[];
  dooSection:any=[];
  ispCertificateSection:any=[];
releivingSection:any=[];
loaSection:any=[];
...
if(response && response.data && response.data.resourceBgvVendordetail){
  this.bgvVendorList= response.data.resourceBgvVendordetail;
      ...
      if (response && response.data && response.data.resourceObSowjdOrderDetail) {
        ...
        const specificExpectedDojDate =new Date(findExpectedDoj +' 00:00:00');// new Date('2024-05-21 00:00:00');
    ...
    this.filenameBGVReport = new URL(this.bgvCompletedDetailsList.bgvReportUrl).pathname.split('/').pop();
      ...
      if (response && response.data && response.data.resourceOBDocumentDetail) {
        ...
        this.loaFileGet = this.onboardingUploadFileList.filter((v: any) => { return v.resourceType == "LOUDocument" })[0];
...
if (response && response.data && response.data.resourceObSendbackRemarkDetail) {
  this.sendBackSectionWise = response.data.resourceObSendbackRemarkDetail;
      ...
      if(this.getElementData.statusdescription =='Submitted' || this.getElementData.statusdescription =='First Level Approved' || this.getElementData.statusdescription=='Second Level Sent Back'){
        ...
        this.addressproofInfoSection = this.sendBackSectionWise?.filter((v: any) => { return v.section == "AddressProofSection" })[0];
        this.residenceSection = this.sendBackSectionWise?.filter((v: any) => { return v.section == "ResidenceSection" })[0];
        this.sscSection= this.sendBackSectionWise?.filter((v: any) => { return v.section == "SSCSection" })[0];
        this.hqSection=this.sendBackSectionWise?.filter((v: any) => { return v.section == "HQSection" })[0];
        ...
        this.purchaseOrderSection=this.sendBackSectionWise?.filter((v: any) => { return v.section == "PurchaseOrderSection" })[0];
        this.louSection=this.sendBackSectionWise?.filter((v: any) => { return v.section == "LouSection" })[0];
        this.bvrSection=this.sendBackSectionWise?.filter((v: any) => { return v.section == "BvrSection" })[0];
        this.ispTrainingSection=this.sendBackSectionWise?.filter((v: any) => { return v.section == "IspTrainingSection" })[0];
        this.parentOrgSection=this.sendBackSectionWise?.filter((v: any) => { return v.section == "ParentOrgCardSection" })[0];
        this.dooSection=this.sendBackSectionWise?.filter((v: any) => { return v.section == "DOOSection" })[0];
        this.ispCertificateSection=this.sendBackSectionWise?.filter((v: any) => { return v.section == "IspCertificateSection" })[0];
        this.releivingSection=this.sendBackSectionWise?.filter((v: any) => { return v.section == "ReleivingLetterSection" })[0];
        this.loaSection=this.sendBackSectionWise?.filter((v: any) => { return v.section == "LOASection" })[0];
...
if(this.getElementData.statusdescription =='First Level Approved'){
        ...
        if (this.addressproofInfoSection) {
          this.addressProofSectionRemarksModel = this.addressproofInfoSection.remark;
          this.isShowAddreessproofInforemarksButtonSaveCancel=true;
          this.showHideAddressProofDetailsRemarksForSendBack = true;
          this.isSetSendBackButtonVisible=true;
        }
        ...
        if (this.sscSection) {
          this.sscSectionRemarksModel = this.sscSection.remark;
          this.isShowSSCRemarksButtonSaveCancel=true;
          this.showHideSSCRemarksForSendBack = true;
          this.isSetSendBackButtonVisible=true;
        }
      
       if (this.hqSection) {
         this.hqSectionRemarksModel = this.hqSection.remark;
         this.isShowHQRemarksButtonSaveCancel=true;
         this.showHideHQRemarksForSendBack = true;
         this.isSetSendBackButtonVisible=true;
       }
     ...
     if (this.purchaseOrderSection) {
      this.purchaseOrderSectionRemarksModel = this.purchaseOrderSection.remark;
      this.isShowPurchaseOrderRemarksButtonSaveCancel=true;
      this.showHidePurchaseOrderRemarksForSendBack = true;
      this.isSetSendBackButtonVisible=true;
    }
      ...
      if (this.louSection) {
       this.louSectionRemarksModel = this.louSection.remark;
       this.isShowLOURemarksButtonSaveCancel=true;
       this.showHideLOURemarksForSendBack = true;
       this.isSetSendBackButtonVisible=true;
     }


     if (this.bvrSection) {
      this.bvrSectionRemarksModel = this.bvrSection.remark;
      this.isShowBVRRemarksButtonSaveCancel=true;
      this.showHideBVRRemarksForSendBack = true;
      this.isSetSendBackButtonVisible=true;
    }

  
      if (this.ispTrainingSection) {
       this.ispTrainingSectionRemarksModel = this.ispTrainingSection.remark;
       this.isShowIspTrainingRemarksButtonSaveCancel=true;
       this.showHideIspTrainingRemarksForSendBack = true;
       this.isSetSendBackButtonVisible=true;
     }
    ...
    if (this.dooSection) {
     this.dooSectionRemarksModel = this.dooSection.remark;
     this.isShowDOORemarksButtonSaveCancel=true;
     this.showHideDOORemarksForSendBack = true;
     this.isSetSendBackButtonVisible=true;
   }
  ...
  if (this.releivingSection) {
   this.releivingSectionRemarksModel = this.releivingSection.remark;
   this.isShowRelievingRemarksButtonSaveCancel=true;
   this.showHideRelievingRemarksForSendBack = true;
   this.isSetSendBackButtonVisible=true;
 }

 if (this.loaSection) {
  this.loaSectionRemarksModel = this.loaSection.remark;
  this.isShowLOARemarksButtonSaveCancel=true;
  this.showHideLOARemarksForSendBack = true;
  this.isSetSendBackButtonVisible=true;
}
}
...
addressProofInfoSendBackButton1(){
  this.showHideAddressProofDetailsRemarksForSendBack=true;
}
...
purchaseOrderSendBackButton1(){
  ...
  cancelSendBack2(){
    ...
    this.showHideAddressProofDetailsRemarksForSendBack=false;
    this.showHideResidenceRemarksForSendBack=false;
    this.showHideSSCRemarksForSendBack=false;
    this.showHideHQRemarksForSendBack=false;
    ...
    this.showHidePurchaseOrderRemarksForSendBack=false;
    this.showHideLOURemarksForSendBack=false;
    this.showHideBVRRemarksForSendBack=false;
    this.showHideIspTrainingRemarksForSendBack=false;
    this.showHideParentOrgRemarksForSendBack=false;
    this.showHideDOORemarksForSendBack=false;
    this.showHideIspCertificateRemarksForSendBack=false;
    this.showHideRelievingRemarksForSendBack=false;
    this.showHideLOARemarksForSendBack=false;
    this.confirmSendbackbtnShowHide = false;
  }
...
this.isShowAddreessproofInforemarksButtonSaveCancel=true;
  ...
  saveAddressproofRemarkss(){
    this.isShowAddreessproofInforemarksButtonSaveCancel=true;
    this.commonRemarksSectionSave("AddressProofSection", this.addressProofSectionRemarksModel, this.addressproofInfoSection, "Address proof Send Back Remarks Saved Successfully", "Addrress Proof");
  }
  ...
  saveSscRemarkss(){
    this.isShowSSCRemarksButtonSaveCancel=true;
    this.commonRemarksSectionSave("SSCSection", this.sscSectionRemarksModel, this.sscSection, "SSC Details Send Back Remarks Saved Successfully", "SSC");
  }
  saveHQRemarkss(){
    this.isShowHQRemarksButtonSaveCancel=true;
    this.commonRemarksSectionSave("HQSection", this.hqSectionRemarksModel, this.hqSection, "Highest Qualification Send Back Remarks Saved Successfully", "Highest Qualification");
  }
  ...
  savePurchaseOrderRemarkss(){
    this.isShowPurchaseOrderRemarksButtonSaveCancel =true;
    this.commonRemarksSectionSave("PurchaseOrderSection", this.purchaseOrderSectionRemarksModel, this.purchaseOrderSection, "Purchase Order Send Back Remarks Saved Successfully", "Purchase Order");
    }
    saveLOURemarkss(){
    this.isShowLOURemarksButtonSaveCancel =true;
    this.commonRemarksSectionSave("LouSection", this.louSectionRemarksModel, this.louSection, "LOU Send Back Remarks Saved Successfully", "LOU");
    }
    saveBVRRemarkss(){
    this.isShowBVRRemarksButtonSaveCancel =true;
    this.commonRemarksSectionSave("BvrSection", this.bvrSectionRemarksModel, this.bvrSection, "BVR Send Back Remarks Saved Successfully", "BVR");
    }
    saveIspTrainingRemarkss(){
    this.isShowIspTrainingRemarksButtonSaveCancel =true;
    this.commonRemarksSectionSave("IspTrainingSection", this.ispTrainingSectionRemarksModel, this.ispTrainingSection, "ISP Training Send Back Remarks Saved Successfully", "ISP Training");
    }
    ...
    saveDOORemarkss(){
      this.isShowDOORemarksButtonSaveCancel =true;
      this.commonRemarksSectionSave("DOOSection", this.dooSectionRemarksModel, this.dooSection, "DOO Send Back Remarks Saved Successfully", "DOO");
    ...
    saveRelivingRemarkss(){
      this.isShowRelievingRemarksButtonSaveCancel =true;
      this.commonRemarksSectionSave("ReleivingLetterSection", this.releivingSectionRemarksModel, this.releivingSection, "Releiving Letter Send Back Remarks Saved Successfully", "Releiving Letter");
  
    }
    saveLOARemarkss(){
      this.isShowLOARemarksButtonSaveCancel =true;
      this.commonRemarksSectionSave("LOASection", this.loaSectionRemarksModel, this.loaSection, "LOA Send Back Remarks Saved Successfully", "LOA");
  ...
  editAddressProffInfoRemarks(){
this.isShowAddreessproofInforemarksButtonSaveCancel=false;
  }
  editResidenceRemarks(){
    this.isShowResidenceRemarksButtonSaveCancel=false; 
  }
  editSscRemarks(){
    this.isShowSSCRemarksButtonSaveCancel=false;
  }
  editHQRemarks(){
    this.isShowHQRemarksButtonSaveCancel=false;
  }
  editWorkExpRemarks(){
  ...
  editPurchaseOrderRemarks(){
    this.isShowPurchaseOrderRemarksButtonSaveCancel =false;
    }
    
    editLOURemarks(){
    this.isShowLOURemarksButtonSaveCancel =false;
    }
    
    editBVRRemarks(){
    this.isShowBVRRemarksButtonSaveCancel =false;
    }
    
    editIspTrainingRemarks(){
    this.isShowIspTrainingRemarksButtonSaveCancel =false;
    }
    ...
    editDOORemarks(){
      this.isShowDOORemarksButtonSaveCancel =false;
    }
    editIspCertificateRemarks(){
      this.isShowIspCertificateRemarksButtonSaveCancel =false;
    }
    editRelievingRemarks(){
      this.isShowRelievingRemarksButtonSaveCancel =false;
    }
    editLOARemarks(){
      this.isShowLOARemarksButtonSaveCancel =false;
    }
  ...
  sendMailApiCallMethod(btnType: any, _sendMailTo: any, _sendMailCC: any, _mainText: any, buttonTextType: any, teamName: any, successMsg: any,remarkValue:any) {
    ...
    if(btnType=='Approve' && (this.getElementData.statusdescription=='First Level Approved' || (this.getElementData.module=='On-boarding Request' && this.getElementData.status=='First Level Approved'))){
       let internetCheck=this.allOnboardingDetails?.onboardingAddditionalDetails[0]?.isInternetAccessRequired==true?'Yes':'No';
       tableFormat="<style>.trbg{background: #eff1f2} .tclsTempTbl tr td{padding:4px;border:3px solid white} </style><table class='tclsTempTbl'  style='width:auto;border-collapse:collapse;line-height:12px;'><tr><td><b>Onboarding Request ID</b></td> <td>" + this.onboardingFirstApproveList?.onbordingRequestId +"</td></tr><tr class='trbg'><td><b>Identity/User Type</b></td> <td> External </td></tr><tr><td><b>Resource Number</b></td> <td> "+this.allOnboardingDetails?.resourceEmpnumberAndMailId[0]?.employeeNumber+" </td></tr><tr class='trbg'><td><b>First Name</b></td> <td>"+ this.resourceObResourceDetailsData.firstName+"</td></tr><tr><td><b>Last Name</b></td> <td> "+ this.resourceObResourceDetailsData.lastName +" </td></tr><tr class='trbg'><td><b>Vendor SAP ID</b></td> <td>"+this.vendorDetailsData?.vendorId+"</td></tr><tr><td><b>Vendor Name</b></td> <td> "+this.vendorDetailsData?.vendorName+" </td></tr><tr class='trbg'><td><b>Group</b></td> <td>"+this.resourceObOrganizationDetailData?.groupName+"</td></tr><tr><td><b>Department</b></td> <td> "+this.resourceObOrganizationDetailData?.departmentName+" </td></tr><tr class='trbg'><td><b>Create Mail Box</b></td> <td>Yes</td></tr><tr><td><b>External Email Communication</b></td> <td> "+this.allOnboardingDetails?.resourceEmpnumberAndMailId[0]?.officialEmailId+" </td></tr><tr class='trbg'><td><b>Internet Access</b></td> <td>"+internetCheck+"</td></tr><tr><td><b>Date of Joining</b></td> <td>"+moment(this.resourceObSowjdOrderDetailData?.expectedDateOfJoing).format('DD-MMM-YYYY') + " </td></tr><tr class='trbg'><td><b>Account Valid Until</b></td> <td>"+moment(this.resourceObSowjdOrderDetailData?.contractEndDate).format('DD-MMM-YYYY') + "</td></tr><tr><td><b>Record URL</b></td> <td> <a href=" + this._getPathUrl + "/Onboarding?data=onboarding target='_blank'><u>Click here</u></a> </td></tr></table>"
    }
