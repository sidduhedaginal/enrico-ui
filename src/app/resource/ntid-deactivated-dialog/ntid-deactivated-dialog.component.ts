...
@Component({
  selector: 'app-ntid-deactivated-dialog',
  templateUrl: './ntid-deactivated-dialog.component.html',
  styleUrls: ['./ntid-deactivated-dialog.component.scss'],
  providers: [
    {
      provide: DateAdapter,
...
export class NtidDeactivatedDialogComponent implements OnInit {
  viewData: any;
  ...
  constructor(
    public dialogRef: MatDialogRef<NtidDeactivatedDialogComponent>,
    ...
    this.createdForm = this.fb.group({
      ntidDeactivatedDate:['', Validators.required],
      //ntidDeletedDate:['', Validators.required],
      ntidRemarks:['', Validators.required],
    });
  }
  ...
  ngOnInit() {
    ...
    this.minDate=new Date(this.viewData.rowData.lastWorkingDay);
    let obj={
...
if(response && response.status=="success" && response.data && response.data.eclProcessEmailId && response.data.eclProcessEmailId.length>0){
  ...
  submitBtn(createdForm: any) {
    ...
    let _ntidDeactivatedDate = moment(createdForm.controls.ntidDeactivatedDate.value).format('YYYY-MM-DD'); 
   // let _ntidDeletedDate = moment(createdForm.controls.ntidDeletedDate.value).format('YYYY-MM-DD'); 
    let _ntidRemarks = createdForm.controls.ntidRemarks.value;
...
if(this._checkuser){
   _createdBy=this._checkuser.displayName;
    ...
    let _objModel ={
      ...
      "ntidDeactivateDt": _ntidDeactivatedDate,
      "ntidDeleteDt":null,//_ntidDeletedDate,
      "remark":_ntidRemarks,
    ...
    //  subject: 'ENRICO | Deboarding | '+  this.viewData.rowData.deboardingRequestID + ' | NTID Deactivated ',
    //   paraInTemplate: {
    //     teamName: 'Team',
    //     mainText: 'NTID Deactivate successfully  for '+ this.viewData.rowData.fullname + '(' + this.viewData.rowData.employeeNumber +') ...!!'  +"<style>.trbg{background: #eff1f2} .tclsTempTbl tr td{padding:4px;border:3px solid white} </style><table class='tclsTempTbl'  style='width:auto;border-collapse:collapse;line-height:12px;'><tr><td><b>Request ID</b></td> <td>"+ this.viewData.rowData.deboardingRequestID +"</td></tr><tr class='trbg'><td><b>Module</b></td> <td> Deboarding </td></tr><tr><td><b>Owner Name</b></td> <td>"+  this.viewData.rowData.ownerName +" </td></tr></table>",
    ...
    this.API.ntidDeactivatedPost(_objModel).subscribe((response2: any) => {
      this.loaderService.setDisableLoading();
      if(response2.status=="success"){
        this.dialogRef.close('true');
        this.snackBar.open("NTID Deactivated Created Successfully...!", 'Close', {
          duration: 3000,
        });
