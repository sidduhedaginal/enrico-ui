...
@Component({
  selector: 'app-device-collected-dialog',
  templateUrl: './device-collected-dialog.component.html',
  styleUrls: ['./device-collected-dialog.component.scss'],
  providers: [
    {
      provide: DateAdapter,
...
export class DeviceCollectedDialogComponent implements OnInit {
  viewData: any;
  ...
  constructor(
    public dialogRef: MatDialogRef<DeviceCollectedDialogComponent>,
    ...
    this.createdForm = this.fb.group({
      deviceCollectedDate: ['', Validators.required],
      deviceAssestNumber: [''],
      deviceItTicketNumber: [''],
      deviceRemarks: ['', Validators.required]
    });
  }
...
if(response && response.status=="success" && response.data && response.data.eclProcessEmailId && response.data.eclProcessEmailId.length>0){
  ...
  submitBtn(createdForm: any) {
    ...
    let _deviceCollectedDate = moment(createdForm.controls.deviceCollectedDate.value).format('YYYY-MM-DD'); 
    let _deviceAssestNumber = createdForm.controls.deviceAssestNumber.value;
    let _deviceItTicketNumber = createdForm.controls.deviceItTicketNumber.value;
    let _deviceRemarks = createdForm.controls.deviceRemarks.value;
let _createdBy='';
if(this._checkuser){
   _createdBy=this._checkuser.displayName;
    ...
    //   subject: 'ENRICO | Deboarding | '+  this.viewData.rowData.deboardingRequestID + ' | Device Collected ',
    //   paraInTemplate: {
    //     teamName: 'Team',
    //     mainText: 'Device collected from '+ this.viewData.rowData.fullname + '(' + this.viewData.rowData.employeeNumber +') successfully...!!' +"<style>.trbg{background: #eff1f2} .tclsTempTbl tr td{padding:4px;border:3px solid white} </style><table class='tclsTempTbl'  style='width:auto;border-collapse:collapse;line-height:12px;'><tr><td><b>Request ID</b></td> <td>"+ this.viewData.rowData.deboardingRequestID +"</td></tr><tr class='trbg'><td><b>Module</b></td> <td> Deboarding </td></tr><tr><td><b>Owner Name</b></td> <td>"+  this.viewData.rowData.ownerName +" </td></tr></table>",
    ...
    // }; 
      ...
      let _objModel ={
        ...
        "deviceCollectDt":_deviceCollectedDate,
        "assetNumber":_deviceAssestNumber,
        "itTicketNumber": _deviceItTicketNumber,
        "remark": _deviceRemarks,
    ...
    this.API.deviceCollectedPost(_objModel).subscribe((response2: any) => {
      this.loaderService.setDisableLoading();
      if(response2.status=="success"){
        this.dialogRef.close('true');
        this.snackBar.open("Device collected Created Successfully...!", 'Close', {
          duration: 3000,
        });
